...
echo ============================================================
echo                      Qwantej Launcher
echo ============================================================
echo.

:: PID file paths -- written here so stop-Qwantej.bat knows where to look.
...
if not errorlevel 1 (
    echo [INFO] Port 8010 is already in use. Verifying whether it is Qwantej...
    powershell -NoProfile -Command ^
      "$openapi = $null; " ^
      "try { $openapi = Invoke-RestMethod -UseBasicParsing http://localhost:8010/openapi.json -TimeoutSec 5 } catch { } " ^
      "if ($openapi -and $openapi.info -and $openapi.info.title -eq 'Qwantej') { exit 0 } " ^
      "exit 1"
    if errorlevel 1 (
...
powershell -NoProfile -Command ^
  ...
  "    if ($openapi.info.title -eq 'Qwantej') { exit 0 } " ^
  "  } catch { } " ^
  "  Start-Sleep -Seconds 1; " ^
  "} " ^
  "exit 1"
if errorlevel 1 (
    echo [ERROR] Qwantej backend did not become ready within 60 seconds.
    echo         Check the Qwantej-Backend window for startup errors.
    pause & exit /b 1
)
echo       [OK] Qwantej backend is ready.
