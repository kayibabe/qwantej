...
async def compute_signals_for_date(db: AsyncSession, run_date: date) -> int:
    ...
    # Detach fixtures from session before any rollback can expire their attributes.
    # expunge() retains all currently-loaded attributes as plain Python values while
    # removing the objects from the session, preventing MissingGreenlet lazy-reload
    # errors when rollback() is called later in this function.
    for _f in fixtures:
        db.expunge(_f)
